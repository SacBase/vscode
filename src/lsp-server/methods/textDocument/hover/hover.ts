...
import { queryCompilerSymbolsHover } from "$lsp-server/methods/textDocument/navigation/compilerAdapter";
import type { CompilerNavigationRuntimeConfig, HoverDebugLogger } from "$lsp-server/methods/textDocument/navigation/types";
import { readDefinitionDocComment, readDefinitionSignature } from "$lsp-server/parser/navigation/sourceDocs";
import { formatHoverDocumentationMarkdown } from "./hoverDocFormatting";
import { resolveHoverDocumentation } from "./hoverDocs";
import { lookupHoverTarget } from "./index";
...
export async function provideHover(
  ...
  const compilerHover = await queryCompilerSymbolsHover(
    {
      document,
      position,
      workspaceRoot,
