...
import { groupDiagnostics, parseCompilerOutput, presentDiagnostics } from "$lsp-server/methods/textDocument/diagnostics/adapter";
import { buildDiagnosticWithRange } from "$lsp-server/methods/textDocument/diagnostics/range";
import { buildRelatedInformation } from "$lsp-server/methods/textDocument/diagnostics/relatedInfo";
import { createInvocation, isLikelyMessagingFlagFailure, runSac2c, type SacCompilerRunResult } from "$sac2c/invoke";
...
export function createDiagnosticsWorkflow(deps: DiagnosticsWorkflowDeps): DiagnosticsWorkflow {
  ...
  const validateDocument = async (document: TextDocument): Promise<void> => {
    if (!isFileDocument(document)) {
      deps.connection.console.log(`[diagnostics] Skipping non-file document: ${document.uri}`);
      return;
    }
    ...
    deps.connection.console.log(`[diagnostics] Validating ${fsPath}...`);
    ...
    try {
      deps.connection.console.log(`[diagnostics] Publishing ${diagnostics.length} diagnostic(s) for ${document.uri}`);
  ...
  const validateAllWorkspaceSacFiles = async (): Promise<void> => {
    ...
    const excludedDirNames = new Set<string>(settings.workspaceScanExcludeDirectories);
