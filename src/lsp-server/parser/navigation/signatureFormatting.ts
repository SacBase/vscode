import type { NavigationSignatureEntry } from "$lsp-server/parser/navigation/types";
...
export function formatSignature(symbolName: string, signature: NavigationSignatureEntry | null): string | null {
  ...
  const params = signature.signature.parameters.map((param) => `${param.typeShape?.typeRepr ?? "unknown"} ${param.name}`.trim()).join(", ");
  const returnTypes = signature.signature.returns.map((ret) => ret.typeRepr ?? "unknown").join(", ");
