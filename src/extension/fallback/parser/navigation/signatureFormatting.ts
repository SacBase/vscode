import type { NavigationSignatureEntry, NavigationSignatureParameter } from "$extension/fallback/parser/navigation/types";
...
export function formatSignature(symbolName: string, signature: NavigationSignatureEntry | null): string | null {
  ...
  const params = signature.signature.parameters.map((param: NavigationSignatureParameter) => `${param.typeShape?.typeRepr ?? "unknown"} ${param.name}`.trim()).join(", ");
  const returnTypes = signature.signature.returns.map((ret: { typeRepr?: string }) => ret.typeRepr ?? "unknown").join(", ");
  ...
  if (display.length > 0) {
    ...
    const hasStatementMarkers = compact.includes(";") || /\breturn\b/i.test(compact) || /[{}|]/.test(compact);
